...
VolumeLoader.parseDicomBuffers = async function(buffers, onend, oninfo){
	...
	for(var seriesId in series){
		...
		var rescaleIntercept = VolumeLoader.DicomUtils.getValue(serie.images[0],VolumeLoader.DicomUtils.TAGS.rescaleIntercept)[0];
		var rescaleSlope = VolumeLoader.DicomUtils.getValue(serie.images[0],VolumeLoader.DicomUtils.TAGS.rescaleSlope)[0];
		var rescaleType = VolumeLoader.DicomUtils.getValue(serie.images[0],VolumeLoader.DicomUtils.TAGS.rescaleType)[0];
...
VolumeLoader.downloadArrayView = function(view, name = "volume"){
...
VolumeLoader.downloadVL = function(volume){
    ...
    VolumeLoader.downloadArrayView(view8, "volume.vl");
};
