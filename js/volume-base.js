...
Volume.prototype.configure = function(o){
	...
	this._minmax = false;
	//console.log('\nwidth sapcing: ',this.widthSpacing.toFixed(4),'\nheight spacing: ',this.heightSpacing.toFixed(4),'\ndepth spacing: ',this.depthSpacing.toFixed(4));
}

//Set v to voxel at x,y,z.
//	- If c is defined v must be a value
//	- If c is not defined v must be an array of length channels
Volume.prototype.setVoxel = function(x, y, z, v, c){
	var i = (x + this.width*y + this.width*this.height*z)*this.channels;
	if(i >= this._data.length) return false;
	
	//Set all values
	if(c == undefined){
		if(!Array.isArray(v) || v.length != this.channels.length) return false;
		for(var c = 0; i<this.channels; i++){
			this._data[i+c] = v[c];
		}
	}
	//Set value c
	else{
		if(c >= this.channels || isNaN(v)) return false;
		this._data[i+c] = v;
	}
...
//Get voxel at x,y,z
//	- If c is defined it will return of value at that channel
//	- If c is undefined it will return an array of length channels
Volume.prototype.getVoxel = function(x, y, z, c){
	var i = (x + this.width*y + this.width*this.height*z)*this.channels;
	if(i >= this._data.length) return false;
	...
	if(c == undefined){
		var v = [];
		for(var c = 0; i<this.channels; i++){
			v.push(this._data[i+c]);
		}
		return v;
	}else{
		return this._data[i+c];
	}
}
...
Volume.prototype.array2csv = function(){
	var csvRows = [];
	csvRows[0] = "hello";
    for (var i = 1; i < 30; i++) {
             
        csvRows.push(this._data[i]);
    }
...
TransferFunction.prototype.fromPoints = function(points){
	...
	for(var pos=0; pos<=4*this.width; pos+=4){
		...
		this._view[pos  ] = Math.round(r * 255) ;
		this._view[pos+1] = Math.round(g * 255);
		this._view[pos+2] = Math.round(b * 255);
		this._view[pos+3] = Math.round(a * 255);
...
TransferFunction.prototype.create = function(width, values){
...
TransferFunction.prototype.clone = function(tf){
	return TransferFunction.create(tf.width, tf._view);
}

TransferFunction.prototype.addTransferFunction = function(tf, name){
